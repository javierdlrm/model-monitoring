...
import io.hops.ml.monitoring.outliers.detectors.{DescriptiveStatsDetector, StatsOutlierDetector}

case class OutliersConfig(statsBased: Seq[StatsOutlierDetector])

object OutliersConfig {
  implicit val decodeOutlierConfig: Decoder[OutliersConfig] = Decoder.instance { c: HCursor =>
    var detectors = Seq[StatsOutlierDetector]()
