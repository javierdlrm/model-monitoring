...
import io.hops.monitoring.window.{Window, WindowSetting}
import io.hops.monitoring.io.dataframe.DataFrameSource._
import io.hops.monitoring.io.file.FileSink._
import io.hops.monitoring.stats.{DatasetStats, StatsPipeState}
 ...
 */
object IrisMLBuiltInWindowStatsMonitoring {
  ...
  def main(args: Array[String]): Unit = {
    ...
    LoggerUtil.log.info(s"[BuiltInWindowStatsMonitoring] Starting job...")
    ...
    val fullPath = ResourcesDir + "builtinstast" + "-" + File.ParquetFormat
    ...
    val (kvgd, schema) = wdf.kvgd(cols)
    val rowEncoder = Encoders.rowEncoder(schema)

    kvgd.flatMapGroups(computeBuiltInStats)(rowEncoder)
      .writeStream
      .format(File.ParquetFormat)
      .option(File.Path, fullPath)
      .option(File.CheckpointLocation, checkpointLocation)
      .outputMode(OutputMode.Append())
      .start().awaitTermination(timeout.milliseconds)

    // Close session
    LoggerUtil.log.info("[BuiltInWindowStatsMonitoring] Shutting down spark job...")
    spark.close()

    def computeBuiltInStats(window: Window, instances: Iterator[Row]): Iterator[Row] = {
      import org.apache.spark.sql.types._
      val spark = SparkSession.getDefaultSession
      LoggerUtil.log.info(s"[BuiltInWindowStatsMonitoring] Spark: ${spark.isDefined}")
      if (spark isDefined) {
        val rdd = spark.get.sparkContext.parallelize(instances.toSeq)
        val schema = StructType(cols.map(StructField(_, StringType)))
        val df = spark.get.sqlContext.createDataFrame(rdd, schema)
        val description = df.describe()
        LoggerUtil.log.info(s"[BuiltInWindowStatsMonitoring] Description: [${description.head(4)}]")
        description.rdd.toLocalIterator
      } else Iterator()
    }
  }
